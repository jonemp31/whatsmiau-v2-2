...
// EmitConnectionEvent dispara um webhook de atualização de status de conexão.
// Esta função é pública para ser chamada durante a inicialização da aplicação.
func (s *Whatsmiau) EmitConnectionEvent(id string, state Status) {
	instance := s.getInstanceCached(id)
	if instance == nil {
		zap.L().Warn("no instance found for connection event", zap.String("instance", id), zap.String("state", string(state)))
		return
	}
	...
	// Reutiliza a lógica interna de disparo de evento.
	s.handleConnectionEvent(id, instance, string(state), eventMap)
}
